...
const configurarPaciente = () => {
  ...
  parseInt(dni).isNaN
    ? errDNI()
    : !(profesionalObj.dni == undefined)
    ? (PacienteObj.dni.push(dni), (indice = PacienteObj.dni.length - 1))
    : PacienteObj.dni.forEach((item) => {
        return item.includes(dni);
      })
    ? (alert("El paciente ya existe"), menuPrincipal())
    : (PacienteObj.dni.push(dni), (indice = PacienteObj.dni.length - 1));
...
let opcion = 1;
PacienteObj.apellido.push("abadin");
PacienteObj.apellido.push("lucio");
// menuPrincipal();
